...
//Ustawienie wielkosci pakietu
    ...
    uint32_t nnode = 100;                                 //nodes number
    uint32_t maxpack = 10000000;                         //max packets
    //char interval[] = "0.02";                           //interval between packets
    double frequency[] = {1.0, 2.0, 5.5, 11.0}; //whether 1.0 2.0 11.0 or 5.1 GHz
int cw_start = 600;
int cw_stop = 601;
 CommandLine cmd;
  cmd.AddValue("nnode", "Number of nodes", nnode);
  cmd.AddValue("cw_start", "Number of Cw start", cw_start);
  cmd.AddValue("cw_stop", "Number of nodes", cw_stop);
  cmd.Parse (argc, argv);
    int   CWmin = 31;      //31
    int   CWmax = 1023;    //1023
for(int CWvalue = cw_start; CWvalue < cw_stop; ++CWvalue){
CWmin = CWmax = CWvalue;
//deklaracja zmiennych z terminala

      for (int i = 0; i < 4; i++) //MCS do 7
{
    double inter = (payloadSize * nnode / frequency[i] * 8) / 1000000.0;
    string interval = to_string(inter);

 

        string Mode[] = {"DsssRate1Mbps", "DsssRate2Mbps", "DsssRate5_5Mbps", "DsssRate11Mbps"};


string phyMode(Mode[i]);
